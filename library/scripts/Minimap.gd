...
func _process(delta):
	...
	for nodepath in markers:
		...
		marker_pos_clamped.x = clamp(
			marker_pos.x, map_margin.x, marker_parent.size.x - map_margin.x - tex_size.x
		)
		marker_pos_clamped.y = clamp(
			marker_pos.y, map_margin.y, marker_parent.size.y - map_margin.y - tex_size.y
		)
		...
		markers[nodepath].scale = (
			Vector2.ONE * clamp(1.0 - (1.0 / vision_offviewport) * distance, 0.0, 1.0)
		)
